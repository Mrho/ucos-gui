...
//  DeviceCode = 0x8999;		   //0x8989	  0x8999
...
// second release on 3/5  ,luminance is acceptable,water wave appear during camera preview
  ...
  //Panel Interface Control//
	...
	else if(DeviceCode==0x8999)
	{
    	...
    	LCD_WriteReg(0x0011,0x6070);    ili9320_Delay(50);        //0x4030           //定义数据格式  16位色 		横屏 0x6058
...
__inline void ili9320_SetWindows(u16 StartX,u16 StartY,u16 EndX,u16 EndY)
{
	...
	{
	  	LCD_WriteReg(0x44, StartX|((StartX+EndX)<<8));	 //水平方向GRAM起始地址(低字节为起始地址，高字节为结束地址)
	  	LCD_WriteReg(0x45, StartY);		  	//垂直方向GRAM起始地址
	  	LCD_WriteReg(0x46, StartY+EndY);  	//垂直方向GRAM结束地址
	}
	else
	{
